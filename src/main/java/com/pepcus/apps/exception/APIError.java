...
import static com.pepcus.apps.constant.ApplicationConstants.VALID_ISO_8601_DATE_FORMAT_1;
...
public class APIError {
   ...
   */
  private APIError() {
    timestamp = ZonedDateTime.now().format(DateTimeFormatter.ofPattern(VALID_ISO_8601_DATE_FORMAT_1));
  }
   ...
   * @param errorMessage
   */
  APIError(HttpStatus status, String errorMessage) {
    this(status);
  ...
  public void addErrorDetail(FieldError fieldError) {
    this.addErrorDetail(fieldError.getObjectName(), fieldError.getField(), fieldError.getRejectedValue(),
  ...
  private void addErrorDetail(ConstraintViolation<?> cv) {
    this.addErrorDetail(cv.getRootBeanClass().getSimpleName(),
  ...
  void addErrorDetail(MethodArgumentTypeMismatchException ex) {
    ...
    if ((ex.getValue() instanceof String) && !(ex.getRequiredType().getName().equals("java.lang.String"))) {
      message = ex.getName() + " must be integer value";
    }
    this.addErrorDetail("requestParameter", ex.getName(), ex.getValue(), message);
  }
