...
public class RequestMatcherProvider {
  ...
  static { // These are static urls, so keep it seperate
    oAuthUrls = new ArrayList<String>(Arrays.asList("/v1/login", "/v1/logout", "/v1/oauth/authorize",
        "/v1/oauth/confirm_access", "/v1/oauth/error", "/v1/oauth/decode_token"));
  }

  /**
   * Get list of request matchers to be ignored by resource server from authorization
   * configurations.
   * 
   * @return
   */
